...
class ARIGID_MLS:
    def __init__(self,img=None,tImg = None,plist = None,qlist = None,a=1.0,preFix = None,gridSize = None,\
                gridArray = None,imgRaw = None,gridArrayRaw = None):
        self.img = img
        self.tImg = tImg
        self.a = a 
        self.plist = plist
        ...
        self.gridArrayRaw = gridArrayRaw
        self.imgRaw = imgRaw
    ...
    def run(self):
        ...
        drawGridPlus(dimg,self.gridSize,self.gridArrayRaw,noShow = False)
        dtImg = showImgPoints(tImg.copy(),self.qlist,winName= 'tar')
        drawGridPlus(dtImg,self.gridSize,self.gridArrayRaw,noShow = False)
            ...
            for j in range(img.shape[1]):
                ...
                for k in range(self.n):
                    ...
                    if norm < 1e-6:
                        ...
                        if i%self.gridSize==0 and j%self.gridSize==0:
                            x = i // self.gridSize
                            y = j // self.gridSize 
                            self.gridArray[x,y] = self.qlist[k]
                        nearControl = True
                        break
                    else:
                        w[k] = 1.0 / norm
                ...
                f_v_vec = np.zeros((1,2),dtype=float)
                for ki in range(self.n):
                    ...
                    f_v_vec += _q_hat[ki].dot(Ai)
                norm_f_v_vec = np.linalg.norm(f_v_vec)
                norm_v_p_star = np.linalg.norm(left_term)
                out = norm_v_p_star * f_v_vec / norm_f_v_vec + q_star
                # print('out',out)
                # input('check')
    ...
    def getDeformImg(self,dept,img,dimg,dtImg,gridSize = 3):
        ...
        drawGridPlus(timg,self.gridSize,gridArray=self.gridArray,noShow = False)
        timg = showImgPoints(timg.copy(),self.qlist,winName= 'deform')
        padImg = np.zeros((dimg.shape[0],10,3),dtype=np.uint8)
        imgCompose = np.hstack([dimg,padImg,dtImg,padImg,timg])

        pad = np.zeros((50,imgCompose.shape[1],3),dtype=np.uint8)
        pad[:] = 255
        imgCompose = np.vstack([pad,imgCompose])
        
        font = cv2.FONT_HERSHEY_SIMPLEX

        cv2.putText(imgCompose,'rigid_mls',(320,30),font,0.8,(0,0,255),2)

        cv2.imshow('imgCompose',imgCompose)
        cv2.imwrite('./data/ani/zk_arigid_{}_large.png'.format(self.preFix),imgCompose)
        cv2.waitKey(20)
...
# img = cv2.resize(img,(img.shape[1]//2,img.shape[0]//2))
# afMls = ARIGID_MLS(img=img_0,tImg = img_1,plist = plist,qlist = qlist.copy())
# afMls.run()
...
# img = cv2.resize(img,(img.shape[1]//2,img.shape[0]//2))
# afMls = ARIGID_MLS(img=img,tImg = img,plist = plist,qlist = qlist.copy())
# afMls.run()
...
gridSize = 20
# img = cv2.resize(img,(img.shape[1]//2,img.shape[0]//2))
gridArray = drawGridPlus(img.copy(),gridSize=gridSize,noShow = False)
gridArrayRaw = gridArray.copy()
# step_list = [float(i)/niter for i in range(niter)]
for i in range(3,niter+1):
    qlist_t = (qlist - plist)*float(i)/niter + plist
    afMls = ARIGID_MLS(img=img,tImg = img,plist = plist,qlist = qlist_t.copy(),preFix = str(i),\
            gridSize = gridSize,gridArray = gridArray,imgRaw = img.copy(),gridArrayRaw = gridArrayRaw)
    afMls.run()